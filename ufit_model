...
class DownSampler(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        return rearrange(x, 'b d (h dh) (w dw) -> b (dh dw) (h w) d', dh=self.down_factor, dw=self.down_factor), \
               rearrange(mask, 'b d (h dh) (w dw) -> b (dh dw) (h w) d', dh=self.down_factor, dw=self.down_factor)
...
class DownSample_Attn(nn.Module):
    ...
    def forward(self, x, mask):
        b, _, h, w = x.size()
        x, down_mask = self.downsampler(x, mask)
        qkv = self.to_qkv(x).masked_fill(~down_mask, 0).chunk(3, dim=-1)
        ...
        if self.attn_type is None: # v1 attention
            ...
            mask_for_attn = ((~down_mask) | (~down_mask).transpose(-2, -1))
            mask_for_attn = (mask_for_attn & (~torch.eye(mask_for_attn.shape[-1], dtype=torch.bool, device=x.device))).unsqueeze(2)
            attn = attn.masked_fill(mask_for_attn, float('-inf'))
        ...
        x = self.to_out(x).masked_fill(~mask, 0)
        return x
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, mask):
        if (not self.rep) or (self.rep and self.training): # debugged: 2024/6/4
            x = self.project_in(x)
            x = F.gelu(x).masked_fill(~mask, 0)
            if self.rep:
                out = (x + self.dwconv[0](x)).masked_fill(~mask, 0)
                for module in self.dwconv[1:]:
                    out = out + module(x)
            else:
                out = self.dwconv(x)
            x = self.project_out(out).masked_fill(~mask, 0)
        else: # eval & rep
            assert False
...
class U_FiTBlock(nn.Module):
    ...
    def forward(self, x, c, mask):
        ...
        x = x + gate_msa.unsqueeze(-1).unsqueeze(-1) * self.attn1(\
            modulate(self.norm11(x), shift_msa, scale_msa), mask).masked_fill(~mask, 0)
        x = x + gate_mlp.unsqueeze(-1).unsqueeze(-1) * self.mlp1(\
            modulate(self.norm21(x), shift_mlp, scale_mlp), mask).masked_fill(~mask, 0)

        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation2(c).chunk(6, dim=1)
        x = x + gate_msa.unsqueeze(-1).unsqueeze(-1) * self.attn2(\
            modulate(self.norm12(x), shift_msa, scale_msa), mask).masked_fill(~mask, 0)
        x = x + gate_mlp.unsqueeze(-1).unsqueeze(-1) * self.mlp2(\
            modulate(self.norm22(x), shift_mlp, scale_mlp), mask).masked_fill(~mask, 0)
        return x
...
class FinalLayer(nn.Module):
    ...
    def forward(self, x, mask, c):
        ...
        x = modulate(self.norm_final(x), shift, scale).masked_fill(~mask, 0)
        x = self.out_proj(x).masked_fill(~mask, 0)
        return x
...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        ...
        self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=2, stride=1, padding=1, bias=bias)
...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.pix_unshfl = nn.PixelUnshuffle(2)

    def forward(self, x, mask):
        return self.pix_unshfl(self.conv(x).masked_fill(~mask, 0)), mask[:, :, ::2, ::2]


def expand_tensor(tensor):
    expanded_h = torch.repeat_interleave(tensor, 2, dim=2)

    expanded_hw = torch.repeat_interleave(expanded_h, 2, dim=3)
...
class Upsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.conv = nn.Conv2d(n_feat, n_feat * 2, kernel_size=3, stride=1, padding=1, bias=False)
        self.pix_shfl = nn.PixelShuffle(2)

    def forward(self, x, mask):
        return self.pix_shfl(self.conv(x).masked_fill(~mask, 0)), expand_tensor(mask)

class U_FiT(nn.Module):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.encoder_level_1 = nn.ModuleList([
            U_FiTBlock(input_size, hidden_size, num_heads, mlp_ratio=mlp_ratio, down_factor=down_factor[0], rep=rep, ffn_type=ffn_type, **kwargs) for _ in range(depth[0])
        ])
        self.down1_2 = Downsample(hidden_size)

        # encoder-2
        self.encoder_level_2 = nn.ModuleList([
            U_FiTBlock(input_size//2, hidden_size*2, num_heads, mlp_ratio=mlp_ratio, down_factor=down_factor[1], rep=rep, ffn_type=ffn_type, **kwargs) for _ in range(depth[1])
        ])
        self.down2_3 = Downsample(hidden_size*2)

        # latent
        self.latent = nn.ModuleList([
            U_FiTBlock(input_size//4, hidden_size*4, num_heads, mlp_ratio=mlp_ratio, down_factor=down_factor[2], rep=rep, ffn_type=ffn_type, **kwargs) for _ in range(depth[2])
        ])
        ...
        self.decoder_level_2 = nn.ModuleList([
            U_FiTBlock(input_size//2, hidden_size*2, num_heads, mlp_ratio=mlp_ratio, down_factor=down_factor[3], rep=rep, ffn_type=ffn_type, **kwargs) for _ in range(depth[3])
        ])
        ...
        self.decoder_level_1 = nn.ModuleList([
            U_FiTBlock(input_size, hidden_size*2, num_heads, mlp_ratio=mlp_ratio, down_factor=down_factor[4], rep=rep, ffn_type=ffn_type, **kwargs) for _ in range(depth[4])
        ])
    ...
    def forward(self, x, t, y, mask=None):
        """
        Forward pass of U-FiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        y: (N,) tensor of class labels
        """
        mask = torch.ones((len(x), 1, x.shape[-1], x.shape[-2])).to('cuda')
        x = self.x_embedder(x, mask)                   # (N, C, H, W)
        ...
        for block in self.encoder_level_1:
            out_enc_level1 = block(out_enc_level1, mask, c1)
        inp_enc_level2, mask = self.down1_2(out_enc_level1, mask)

        # encoder_2
        out_enc_level2 = inp_enc_level2
        for block in self.encoder_level_2:
            out_enc_level2 = block(out_enc_level2, mask, c2)
        inp_enc_level3, mask = self.down2_3(out_enc_level2, mask)

        # latent
        latent = inp_enc_level3
        for block in self.latent:
            latent = block(latent, mask, c3)

        # decoder_2
        inp_dec_level2, mask = self.up3_2(latent, mask)
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2).masked_fill(~mask, 0)
        out_dec_level2 = inp_dec_level2
        for block in self.decoder_level_2:
            out_dec_level2 = block(out_dec_level2, mask, c2)

        # decoder_1
        inp_dec_level1, mask = self.up2_1(out_dec_level2, mask)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        inp_dec_level1 = self.reduce_chan_level1(inp_dec_level1).masked_fill(~mask, 0)
        out_dec_level1 = inp_dec_level1
        for block in self.decoder_level_1:
            out_dec_level1 = block(out_dec_level1, mask, c2)

        # output
        x = self.output(out_dec_level1).masked_fill(~mask, 0)

        x = self.final_layer(x, mask, c2)                # (N, T, patch_size ** 2 * out_channels)
...
#                                   U-FITs Configs                                  #
#################################################################################
def U_FiT_custom(**kwargs):
    return U_FiT(**kwargs)

def U_FiT_S(**kwargs):
    return U_FiT(down_factor=2, hidden_size=96, num_heads=4, depth=[2,5,8,5,2], ffn_type='rep', rep=1, mlp_ratio=2, attn_type='v2', posemb_type='rope2d', downsampler='dwconv5', down_shortcut=1, **kwargs)

def U_FiT_B(**kwargs):
    return U_FiT(down_factor=2, hidden_size=192, num_heads=8, depth=[2,5,8,5,2], ffn_type='rep', rep=1, mlp_ratio=2, attn_type='v2', posemb_type='rope2d', downsampler='dwconv5', down_shortcut=1, **kwargs)

def U_FiT_L(**kwargs):
    return U_FiT(down_factor=2, hidden_size=384, num_heads=16, depth=[2,5,8,5,2], ffn_type='rep', rep=1, mlp_ratio=2, attn_type='v2', posemb_type='rope2d', downsampler='dwconv5', down_shortcut=1, **kwargs)

UFiT_models = {
    'U-FiT-custom': U_FiT_custom,
    'U-FiT-S': U_FiT_S, 
    'U-FiT-B': U_FiT_B,
    'U-FiT-L': U_FiT_L, 
}